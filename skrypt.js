...
const niedziela = 1 - day;
    ...
    for (let i = 1; i <= l; i++) {
        if (i == niedziela || i == niedziela + 7 || i == niedziela + 14 || i == niedziela + 21 || i == niedziela + 28 || i == niedziela + 35 || i == 1 && (miesiac == "styczeń" || miesiac == "maj" || miesiac == "listopad") || i == 6 && miesiac == "styczeń" || i == 3 && miesiac == "maj" || i == 15 && miesiac == "sierpień" || i == 11 && miesiac == "listopad" || i == 25 && miesiac == "grudzień" || i == 26 && miesiac == "grudzień") {
            document.getElementById("tabela").innerHTML += "<tr class='suncolor'><td >" + i + "." + "</td><td >" + i + "." + nummie + "." + rok + `</td><td id=tdod${i}><input onchange=aktu() id=od${i} type='time' value=${localStorage.getItem(`sod${i}`) === null ? '' : localStorage.getItem(`sod${i}`)}></input></td><td id=tddo${i}><input onchange=aktu() id=do${i} type='time' value=${localStorage.getItem(`sdo${i}`) === null ? '' : localStorage.getItem(`sdo${i}`)}></input></td><td id=s${i}></td><td > </td><td id=tduw${i}><input class='uwagi' onchange=aktu() list='uwagi_nie' id=uw${i} type='text' value=${localStorage.getItem(`suw${i}`) === null ? '' : localStorage.getItem(`suw${i}`)}></input><datalist id='uwagi_nie'><option value='Ch(L4)'><option value='Uojc'><option value='Op'></datalist></td></tr>`
        }


        else if (i == sobota || i == sobota + 7 || i == sobota + 14 || i == sobota + 21 || i == sobota + 28) {
            document.getElementById("tabela").innerHTML += "<tr class='satcolor'><td style='width:5%'>" + i + "." + "</td><td >" + i + "." + nummie + "." + rok + `</td><td id=tdod${i}><input onchange=aktu() id=od${i} type='time' value=${localStorage.getItem(`sod${i}`) === null ? '' : localStorage.getItem(`sod${i}`)}></input></td><td id=tddo${i}><input onchange=aktu() id=do${i} type='time' value=${localStorage.getItem(`sdo${i}`) === null ? '' : localStorage.getItem(`sdo${i}`)}></input></td><td id=s${i}></td><td > </td><td id=tduw${i}><input class='uwagi' onchange=aktu() list='uwagi_sob' id=uw${i} type='text' value=${localStorage.getItem(`suw${i}`) === null ? '' : localStorage.getItem(`suw${i}`)}></input><datalist id='uwagi_sob'><option value='Ch(L4)'><option value='Uojc'><option value='Op'></datalist></td></tr>`
        }

        else {
            document.getElementById("tabela").innerHTML += `<tr><td>${i}.</td><td>${i}.${nummie}.${rok}</td><td id=tdod${i}><input onchange=aktu() id=od${i} type='time' value=${localStorage.getItem(`sod${i}`) === null ? '' : localStorage.getItem(`sod${i}`)}></input></td><td id=tddo${i}><input onchange=aktu() id=do${i} type='time' value=${localStorage.getItem(`sdo${i}`) === null ? '' : localStorage.getItem(`sdo${i}`)}></input></td><td id=s${i}></td><td > </td><td id=tduw${i}><input class='uwagi' onchange=aktu() list='uwagi' id=uw${i} type='text' value=${localStorage.getItem(`suw${i}`) === null ? '' : localStorage.getItem(`suw${i}`)}></input><datalist id='uwagi'><option value='Ch(L4)'><option value='Uw'><option value='Uż'><option value='Uok'><option value='Uojc'><option value='Uwych'><option value='Urodz'><option value='Ub'><option value='Um'><option value='Op'><option value='Zw.art_188_kp'><option value='NUN'><option value='NUP'></datalist></td></tr>`
        }
...
function zapisz() {
    ...
    document.title = `${pracownik}_karta_godz`;
    ...
    for (k = 1; k <= l; k++) {
        ...
        if (uwvalue === 'Uw' || uwvalue === 'Ch(L4)' || uwvalue === 'Uż' || uwvalue === 'Uok' || uwvalue === 'Uojc' || uwvalue === 'Uwych' || uwvalue === 'Urodz' || uwvalue === 'Ub' || uwvalue === 'Um' || uwvalue === 'Op' || uwvalue === 'Zw.art_188_kp' || uwvalue === 'NUN' || uwvalue === 'NUP') { result = 0; document.getElementById(`tdod${k}`).innerHTML = '— —'; document.getElementById(`tddo${k}`).innerHTML = '— —'; res.innerHTML = '—' };
        sum = sum + result;
...
// pętla licząca sumę godzin dla zmian w inputach
function count() {
    let k;
    let t1;
    let t2;
    ...
    for (k = 1; k <= l; k++) {
        ...
        if (isNaN(t1) || isNaN(t2)) { t1 = 0; t2 = 0 };
        result = Math.round((t2 - t1) * 100) / 100;
        let res = document.getElementById(`s${k}`);
        if (uwvalue === 'Uw' || uwvalue === 'Ch(L4)' || uwvalue === 'Uż' || uwvalue === 'Uok' || uwvalue === 'Uojc' || uwvalue === 'Uwych' || uwvalue === 'Urodz' || uwvalue === 'Ub' || uwvalue === 'Um' || uwvalue === 'Op' || uwvalue === 'Zw.art_188_kp' || uwvalue === 'NUN' || uwvalue === 'NUP') { result = 0 };
        sum = Math.round((sum + result) * 100) / 100;
    }
    return sum;
}

function clean() {
    if (confirm("Czy na pewno chcesz usunąć zapamiętane DANE dla formularza?")) {
        localStorage.clear();
        for (let j = 1; j <= l; j++) {
